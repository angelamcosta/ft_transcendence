...
export default async function userRoutes(fastify) {
	...
	fastify.delete('/users/friends/cancel/:id', {
		...
		try {
			const row = await db.get(`SELECT user_id FROM friends WHERE (user_id = ? AND friend_id = ?) AND friendship_status = 'pending'`, [userId, paramId]);
	...
	fastify.delete('/users/invite/cancel/:id', {
		...
		try {
			const row = await db.get(`SELECT user_id FROM match_invites WHERE (user_id = ? AND friend_id = ?) AND invite_status = 'pending'`, [userId, paramId]);

			if (row) {
				await db.run(`DELETE FROM match_invites WHERE (user_id = ? AND friend_id = ?) AND invite_status = 'pending'`, [userId, paramId]);
				return res.code(200).send({ message: 'Match invite canceled' });
			} else
				return res.code(200).send({ message: 'Match invite not found' })
		} catch (err) {
			if (err.statusCode && err.statusCode !== 500)
				throw err;
			fastify.log.error(`Database error: ${err.message}`);
